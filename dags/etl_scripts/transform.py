...
LOAD_DATA_QUERY = """
SELECT * FROM lacrimeanalysis WHERE date_rptd BETWEEN '{}' and '{}'
"""
...
def load_data(start_date, end_date, database_connection_string, spark_context):
    conn = create_engine(database_connection_string)
    df = pd.read_sql(text(LOAD_DATA_QUERY.format(start_date, end_date)), con=conn)
    # df = df.rename(columns={v: k for k, v in API_TO_DB_COLUMNS.items()})
    # Spark expects iteritems but pandas newer version has items
    df = df.dropna(axis='columns', how='all')
    print(f"Loaded dataframe with shape {df.shape}")
    df.iteritems = df.items
    return spark_context.createDataFrame(df)


def transform_data(spark_master, database_connection_string, app_name = "CrimeDataAnalysis", **context):
    # Initialize Spark session
    spark = SparkSession.builder.appName(app_name).master(spark_master).getOrCreate()

    start_date = (datetime.strptime(context["ds"], "%Y-%m-%d") - relativedelta(months=1)).strftime("%Y-%m-%d")
    end_date = context["ds"]
    ...
    crime_data = load_data(start_date, end_date, database_connection_string, spark)
